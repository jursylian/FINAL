...
const MONGO_URI = "mongodb://localhost:27017/ichgramm";
...
async function main() {
  ...
  const existing = await User.findOne({ username: "ichgramm" });
  if (existing) {
    await Post.deleteMany({ authorId: existing._id });
    await User.deleteOne({ _id: existing._id });
  }

  const stamp = Date.now().toString().slice(-6);
  const username = "ichgramm";
  const email = `ichgramm_${stamp}@example.com`;
